PathModel <- function(taxonomic.table, meta, response, binomial.response = F, variables, readcount.cutoff = 0, 
    subject.ID = NULL,  outlier.cutoff, #p.cutoff = 0.05,
   ...
   rownames(bac_test[[i]]) <- background.variables
    ...
    #sigi <- rownames(bac_test[[i]])[bac_test[[i]][,2]<p.cutoff&!is.na(bac_test[[i]][,2])]
...
palette(c("gray","white","yellow", "yellowgreen", "pink","skyblue","darkorange", "turquoise2","plum", "red", "gray","royalblue","lightyellow",
                  "darkorange",   "olivedrab4", "red", "turquoise4",  "lightyellow4", "black"))

if(pdf) {
  pdf(paste("PathModel_",response,".pdf",sep=""))
qgraph::qgraph(input=res,asize=5,vsize=10,esize=10,directed=T,colFactor=0,borders=T,arrowAngle=0.6,
...
if(quartz) quartz()
qgraph::qgraph(input=res,asize=5,vsize=8,esize=10,directed=T,colFactor=0,borders=T,arrowAngle=0.6,
           labels=substr(namesres,start=1,stop=15),groups=class,color=color,legend=F,
          curveShape=-0.5,curveScale=T,
          curve=curves, edge.label.cex=1,edge.labels=T)
  
} else print(paste("No variables achieved p-value <", p.cutoff))
    }
