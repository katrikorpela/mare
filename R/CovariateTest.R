CovariateTest <- function(taxonomic.table, meta, covariate, readcount.cutoff = 0, 
    ...
    if (length(group) != 0) {
        ...
        dataset2 <- data.frame(metadata, log((taxa2/metadata$ReadCount) * 100+0.0001))
        ...
        if (length(sig) > 0) {
            ...
            p <- ggplot2::ggplot(df, ggplot2::aes(y = value, x = x), environment = environment()) + 
               ggplot2::stat_smooth(data=df[df$value>(log(0.0001)),],method = "lm", formula = y ~ x, se = T, fill = "cornflowerblue", color = "gray30", lwd = 0.5) +
