PCoA <- function(taxonomic.table, meta, readcount.cutoff = 0, group = NULL, group2 = NULL,
    components = c(1, 2), background.variable = NULL, colour.scheme = heat.colors, legendplace = "topright", 
    select.by = NULL, select = NULL, relative = F, pdf = F, quartz = T, keep.result = F) {
    ...
    if (length(group2) != 0) {
        ...
        groupvar2 <- as.numeric(ordered(groupvar2))-1
    } else groupvar2 <- 0
    ...
    for(i in rownames(pctest)) pctest[i,paste("MDS",components[2],sep="")]<-cor(pc[,paste("MDS",components[2],sep="")],pc[,i])
    ...
    if (length(background.variable) != 0) {
        ...
        if (pdf) {
            ...
            if (length(group) != 0) {
                points(spcoa$sites[, components[2]] ~ spcoa$sites[, components[1]], pch = 21 + groupvar2,  bg = as.factor(meta[,group]), col="white")
                legend(legendplace, legend = levels(meta[,group]), title = group, pch = 21, pt.bg = c(1:length(levels(meta[,group]))),  bty = "n",col="white")
                if(length(group2)!=0){
                legend(legendplace,inset=c(0.2,0),legend = levels(meta[,group2]),  title = group2, pch = 21+unique(groupvar2)[order(unique(groupvar2))], col="black",pt.bg = "white",  bty = "n")
                }
               ...
               if(length(group2) != 0) {
                ...
                points(spcoa$sites[, components[2]] ~ spcoa$sites[,components[1]], pch = 21, bg = "black",col="white")
            }
